...
#===============================================
# 修仙世界 Telegram Bot - 无Redis版本部署脚本
# Redis是可选组件,不影响核心功能
#===============================================
...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        ...
        echo "请使用: sudo bash deploy_without_redis.sh"
        exit 1
    fi
}

install_system_dependencies() {
    print_header "1. 安装系统依赖 (不含Redis)"
    ...
    print_success "系统依赖安装完成 (跳过Redis)"
}
...
configure_environment() {
    print_header "6. 配置环境变量 (无Redis版本)"
...
EOF
    ...
    print_success "环境变量配置完成 (Redis已禁用)"
}
...
[Unit]
Description=修仙世界 Telegram Bot (无Redis)
...
show_summary() {
    ...
    echo ""
    echo -e "${YELLOW}注意: Redis缓存已跳过 (不影响核心功能)${NC}"
    echo ""
    echo "服务信息:"
    ...
    echo ""
    echo -e "${GREEN}如需添加Redis支持,参考 PRODUCTION_DEPLOYMENT.md${NC}"
    echo ""
}

main() {
    clear
    print_header "修仙世界 Telegram Bot - 无Redis版部署"

    echo -e "${YELLOW}此脚本将执行以下操作:${NC}"
    echo "  1. 安装系统依赖 (Python, PostgreSQL)"
    echo "  2. 创建服务用户"
    echo "  3. 配置数据库"
    echo "  4. 部署应用程序"
    echo "  5. 配置systemd自动启动"
    echo ""
    echo -e "${YELLOW}注意: 跳过Redis缓存 (不影响核心功能)${NC}"
    echo -e "${RED}请确保你有Telegram Bot Token${NC}"
    echo ""
